...
export default class QuillResize {
  ...
  constructor(quill, options = {}) {
    ...
    this.quill.container.style.position =
      this.quill.container.style.position || "relative";
  ...
  initializeModules() {
    ...
    this.modules = this.moduleClasses.map(
      (ModuleClass) =>
        new (this.constructor.Modules[ModuleClass] || ModuleClass)(this),
    );
  ...
  initializeEmbed() {
    ...
    let cssText = [""]
      .concat(this.options.embedTags)
      .join(`, .${this.embedClassName} `)
      .slice(2);
  ...
  addBlotsSelectedClass(range, oldRange) {
    ...
    const leaves = this.quill.scroll.descendants(
      Parchment.Leaf || Parchment.LeafBlot,
      range.index,
      range.length,
    );
